<template>
    <nav class="bg-white shadow-md border-b border-gray-300">
            ...
            <div class="flex justify-between items-center h-16">
                ...
                <div class="hidden md:block relative" ref="dropdown">
                    <div @click="toggleDropdown"
                        ...
                        <span class="text-gray-700 font-medium">{{ user.fullname || "ชื่อผู้ใช้งาน"}}</span>
                ...
                <div class="md:hidden">
                    <button @click="toggleMobileMenu"
                        class="mobile-menu-button focus:outline-none focus:bg-gray-200 p-2 rounded-md">
        ...
        <div v-show="isMobileMenuOpen"
            class="mobile-menu absolute w-full py-2 md:hidden bg-white shadow-md border-t border-gray-300"
            ref="mobileMenu">
            <div class="flex items-center space-x-2 p-4 text-sm">
                <i class="fa-regular fa-user text-sm"></i>
                <span class="text-gray-700 font-medium">{{ user.fullname || "ชื่อผู้ใช้งาน"}}</span>
            </div>
            <button @click="logout"
                class="block w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-gray-100 duration-200">
                <i class="fa-solid fa-right-from-bracket mr-2"></i>ออกจากระบบ
            </button>
        </div>
    </nav>
...
export default {
    ...
    methods: {
        ...
        getDataUser() {
            ...
            this.user.fullname = localStorage.getItem("loadingJobFullname");
            if (storedHash) {
